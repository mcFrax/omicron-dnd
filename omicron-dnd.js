...
// If selectstart is not supported, we run a poor-man's alternative
// by repeatedly removing the selection. This is the interval for that.
let removeSelectionIntervalId = 0;
...
function initDragContainer(containerEl, options) {
    ...
    toggleListeners(true, containerEl, [
        ...
        ['mousedown', anyState_container_MouseDown],
...
function toggleEvents_statePreDragAndDrag(toggleOn) {
    // Common handlers for both preDrag and drag states.
    if (touchDrag) {
    ...
    toggleListeners(toggleOn, document, [
        ...
        ['auxclick', stopEventPropagation],
        ['pointerdown', stopEventPropagation],
        ['pointerup', stopEventPropagation],
        ['pointercancel', stopEventPropagation],
    ]);
}
...
function toggleEvents_stateDrag(toggleOn) {
    if (touchDrag) {
        toggleListeners(toggleOn, document, [
            ...
            // touchmove has a common handler for preDrag and drag.
    ...
    if ('onselectstart' in document) {
        ...
        if (toggleOn && !removeSelectionIntervalId) {
            removeSelectionIntervalId = setInterval(removeSelection, 20);
        }
        if (!toggleOn && removeSelectionIntervalId) {
            clearInterval(removeSelectionIntervalId);
            removeSelectionIntervalId = 0;
        }
    }
}
...
function anyState_container_TouchDown(event) {
    ...
    //         originalEvents[0].pointerType === 'touch' &&
    //         originalEvents[0].target === event.target) {
    //     // Assume this is the oiginal touchdown.
...
function anyState_container_MouseDown(event) {
    ...
    //         originalEvents[0].pointerType === 'mouse' &&
    //         originalEvents[0].target === event.target) {
    //     // Assume this is the oiginal mousedown.
    //     originalEvents.push(event);
    // }
}
...
function startPreDrag(event, evPlace) {
    ...
    toggleEvents_statePreDragAndDrag(true);
    ...
    // NOTE: Even if we decide to startDrag without a delay, we should do it
    // in setTimeout(..., 0) and not synchronously, so that we can catch the
    // related touchdown event, if any.
...
function exitDrag(execSort) {
    ...
    toggleEvents_statePreDragAndDrag(false);
