...
let htmlOvescrollBehavior: string;
let bodyOvescrollBehavior: string;
let bodyUserSelect: string;
let bodyWebkitUserSelect: string;
let bodyWebkitTouchCallout: string;
...
let knownScrollers: Map<HTMLElement, ScrollerRecord> = new Map();
...
let lastScrollAnimationTimestamp: DOMTimeStamp|null = null;
...
const hoverContainersByDepth: ContainerData[] = [];
...
function initDragContainer(containerEl: HTMLElement, options: Partial<ContainerOptions>) {
    ...
    (containerEl as any)[expando] = containerData;
...
function toggleEvents_statePreDrag(toggleOn: boolean) {
    if (touchDrag) {
...
function toggleEvents_stateDrag(toggleOn: boolean) {
    if (touchDrag) {
        toggleListeners(toggleOn, document, [
...
function cancelIfOmicronActive(event: Event) {
...
function cancelIfCancellable(event: Event) {
    if (event.cancelable) {
        event.preventDefault();
    }
}
